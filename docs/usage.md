...
toggle_choicemodule_ally = false       # Phase 4: set true to enable the Ally assist system
toggle_choicemodule_ally_name = 파티원  # Phase 4: ally character name shown in OOC messages
```
...
### `toggle_choicemodule_ally` *(Phase 4)*

보조(Ally) 판정 시스템 활성화 여부입니다.

- `false` (기본): 비활성 — 기존 동작 유지
- `true`: 활성 — 유저 실패 시 파티원 보조 주사위 발동

### `toggle_choicemodule_ally_name` *(Phase 4)*
...
- `toggle_choicemodule_ally = true`
- 유저의 outcome이 **Narrow Failure / Failure / Critical Failure** (레벨 0–2)
- 보조 주사위 결과가 **Narrow Success 이상** (레벨 3–5)
