...
function onOutput(triggerId, output)
  ...
  if director then
    local tension = director:match('level=%s*[{`"\']*%s*(%w+)')
    if tension then
      setChatVar(triggerId, "ChoiceModule.auto_tension", tension)
    end

    -- ally_name: first char must be non-whitespace/non-delimiter, rest allows spaces (for multi-word names)
    local ally_name = director:match('name=%s*[{`"\']*%s*([^`"\'}>%s][^`"\'>}]*)')
    local ally_avail = director:match('available=%s*[{`"\']*%s*(%w+)')
    if ally_avail then
    ...
    local dc_min = director:match('suggested_min=%s*[{`"\']*%s*(%d+)')
    local dc_max = director:match('suggested_max=%s*[{`"\']*%s*(%d+)')
...
- **Director 파싱**: `<Director>...</Director>` 블록이 있으면 `tension`, `ally`, `ally_name`, `dc_min`, `dc_max` 값을 `ChoiceModule.auto_` 접두사를 가진 chatVar에 저장하고, 출력에서 블록을 제거합니다.
